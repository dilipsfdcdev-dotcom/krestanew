      ...
      <div className="container-luxury" ref={ref}>
        ...
        <div className="grid lg:grid-cols-5 gap-12 lg:gap-16">
          <div className={`reveal-left ${visible ? 'visible' : ''} lg:col-span-2`}>
                ...
                const Inner = (
                  <div className="group flex items-start gap-4">
                    <div className="w-10 h-10 rounded-xl icon-premium flex items-center justify-center flex-shrink-0">
                      <info.icon className="w-4 h-4 text-[#c9a84c]" />
                    </div>
                    <div>
                      <p className="text-white/25 text-xs tracking-widest uppercase mb-1 font-[family-name:var(--font-body)]">{info.title}</p>
            ...
            <div className={`reveal ${visible ? 'visible' : ''} stagger-5 p-6 bg-gradient-to-br from-[#25d366]/15 to-[#128c7e]/8 rounded-2xl border border-[#25d366]/15`}>
              <div className="flex items-center gap-3 mb-4">
                <MessageCircle className="w-8 h-8 text-[#25d366]" />
                <div>
                  <h4 className="font-semibold text-white text-sm">WhatsApp</h4>
                  <p className="text-white/35 text-xs">Instant replies</p>
                </div>
              </div>
          ...
          <div className={`reveal-right ${visible ? 'visible' : ''} lg:col-span-3`}>
            <div className="glass rounded-3xl p-8 md:p-10">
              <h3 className="text-xl font-semibold text-white mb-8 font-[family-name:var(--font-heading)]">Send us a message</h3>
              {isSubmitted ? (
                <div className="flex flex-col items-center justify-center py-16">
                  <CheckCircle className="w-14 h-14 text-[#c9a84c] mb-4" />
                  <h4 className="text-lg font-semibold text-white mb-2 font-[family-name:var(--font-heading)]">Message Sent!</h4>
                  <p className="text-white/35 text-sm">Redirecting to WhatsApp...</p>
                </div>
              ) : (
                <form onSubmit={handleSubmit} className="space-y-5">
                  <div className="grid sm:grid-cols-2 gap-5">
                    <div>
                      <label className="block text-[11px] tracking-widest uppercase text-white/25 mb-2 font-[family-name:var(--font-body)]">Full Name *</label>
                      <input type="text" required value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="w-full px-4 py-3.5 rounded-xl text-white text-sm input-premium placeholder:text-white/15" placeholder="Your name" />
                    </div>
                    <div>
                      <label className="block text-[11px] tracking-widest uppercase text-white/25 mb-2 font-[family-name:var(--font-body)]">Phone *</label>
                      <input type="tel" required value={formData.phone} onChange={(e) => setFormData({...formData, phone: e.target.value})} className="w-full px-4 py-3.5 rounded-xl text-white text-sm input-premium placeholder:text-white/15" placeholder="+91 XXXXX XXXXX" />
                    </div>
                  </div>
                  <div>
                    <label className="block text-[11px] tracking-widest uppercase text-white/25 mb-2 font-[family-name:var(--font-body)]">Email</label>
                    <input type="email" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} className="w-full px-4 py-3.5 rounded-xl text-white text-sm input-premium placeholder:text-white/15" placeholder="your@email.com" />
                  </div>
                  <div>
                    <label className="block text-[11px] tracking-widest uppercase text-white/25 mb-2 font-[family-name:var(--font-body)]">Interested Plot Size</label>
                    <select value={formData.plotSize} onChange={(e) => setFormData({...formData, plotSize: e.target.value})} className="w-full px-4 py-3.5 rounded-xl text-white text-sm input-premium appearance-none">
                      ...
                      <option value="Custom" className="bg-[#111111]">Custom Size</option>
                    </select>
                  </div>
                  <div>
                    <label className="block text-[11px] tracking-widest uppercase text-white/25 mb-2 font-[family-name:var(--font-body)]">Message</label>
                    <textarea rows={4} value={formData.message} onChange={(e) => setFormData({...formData, message: e.target.value})} className="w-full px-4 py-3.5 rounded-xl text-white text-sm input-premium resize-none placeholder:text-white/15" placeholder="Tell us about your requirements..." />
                  </div>
                  <button type="submit" className="w-full py-4 bg-[#c9a84c] text-[#080808] rounded-xl font-semibold text-sm tracking-wide hover:bg-[#e8d48b] transition-all flex items-center justify-center gap-2 btn-premium shimmer">
